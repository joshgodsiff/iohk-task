...
runPure ::
  -- | The agents to run.
  [Agent msg ()] ->
  -- | A list of all broadcasts, represented by
  --  pairs containing a timestamp and the message that was sent.
  [(Natural, msg)]
runPure =
  zip [1 ..] . step . partitionAgents . S.fromList
    . fmap
      (\a -> PureAgent {queue = S.Empty, agent = a})
...
enqueueMsg ::
  msg ->
  PartitionedAgents (PureAgent msg a) ->
  PartitionedAgents (PureAgent msg a)
...
partitionAgent ::
  (c -> Agent msg a) -> c -> PartitionedAgents c -> PartitionedAgents c
...
popAgent pa@PartitionedAgents {broadcasting = (n :<| nx)} =
  (pure n, pa {broadcasting = nx})
popAgent pa@PartitionedAgents {receiving = (n :<| nx)}
  | not . S.null $ queue n = (pure n, pa {receiving = nx})
popAgent pa@PartitionedAgents {delayed = (n :<| nx)} =
  (pure n, pa {delayed = nx})
